...
bool validator::ConstraintsLoading::checkLoadingConstraints(Solution& solution, const ConstraintSet& cSet, Instance& instance, const bool& msg) {
	...
	for (auto& tour : solution.tours) {
		...
		for (const auto item_id : tour.item_ids) {
			auto& currentItem = instance.items.at(item_id);
			if (!checkVehicleWalls(currentItem, instance.vehicle, msg)
				|| !checkMaxCoordinates(currentItem, msg)
				|| !checkDimensions(currentItem, msg)
				|| !checkOverlapping(currentItem, tour, endPos, instance, msg)
				|| !checkAxleWeights(currentItem, tour, cSet.axle_weights, instance, msg)
				|| !checkBalancedLoading(currentItem, tour, cSet.balanced_loading, cSet.balanced_part, instance, msg)
				|| !checkUnloadingSequence(currentItem, tour, endPos, cSet.uSequence, instance, msg)
				|| !checkReachability(currentItem, tour, cSet.lambda * scaleFactor, cSet.reachability, instance, msg)
				|| !checkVStability(currentItem, cSet.vStability, cSet.alpha, instance, msg)
				|| !checkStacking(currentItem, tour, endPos, cSet.stacking, instance, msg)) {
				if (msg) std::cerr << "Solution is NOT feasible according Loading Constraints." << std::endl;
				tour.feasible = false;
				return false;
			}
		}
	...
	if (msg) std::cout << "Solution is feasible according Loading Constraints." << std::endl;
	return true;
}
...
bool validator::ConstraintsLoading::checkVStability(Item& item, const VerticalStability& vStability, const float& alpha, Instance& instance, const bool& msg) {
	...
	if (!checkFlyingItem(item, instance, msg)) return false;
	...
	case VerticalStability::StaticStabilityRamos:
		if (msg) std::cerr << "Not included in this solution validator. Please use other repository.";
		return true;
...
bool validator::ConstraintsLoading::checkFlyingItem(Item& item, Instance& instance, const bool& msg) {
	...
	for (const auto& belowId : item.itemsBelow) {
		const Item& below = instance.items.at(belowId);
		if (below.isBelow(item, true)) {
			return true;
		}
	}

	if (msg) std::cerr << "Item " << item.id << " is flying" << std::endl;
	return true;
}
...
bool validator::ConstraintsLoading::checkStaticStabiltyMack(Item& item, const float& alpha, Instance& instance, const bool& msg) {
	bool flagCG = false;
	...
	for (const auto& belowId : item.itemsBelow) {
		Item& below = instance.items.at(belowId);
		if (below.min.z == 0) {
			supportArea += calcSupportArea(item, below);
		}

		if (below.isBelow(item, true) && 
			(below.min.x <= x && x <= below.max.x && 
			 below.min.y <= y && y <= below.max.y)) {
			flagCG = true;
		}
	}
	...
	if (!flagCG) {
		if (msg) std::cerr << "Item " << item.id << " center of gravity not supported." << std::endl;
		return false;
	}
