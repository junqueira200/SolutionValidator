...
bool validator::ConstraintsRouting::checkRoutingConstraints(Solution& solution, const ConstraintSet& cSet, Instance& instance, const bool& msg) {
	if (!checkTimeWindows(solution, cSet.time_windows, instance, msg)
		...
		if (msg) std::cerr << "Solution is NOT feasible according Routing Constraints." << std::endl;
		return false;
	}
	if (msg) std::cout << "Solution is feasible according Routing Constraints." << std::endl;
	return true;
}

bool validator::ConstraintsRouting::checkCost(const Solution& solution, const Instance& instance, const bool& msg) {
	double totalCost = 0;
	...
	if (timeWindows) {
		...
		for (auto& tour : solution.tours) {
			...
			for (auto customer_id : tour.customer_ids) {
				if (!checkTimeWindowCustomer(customer_id, customer_last_id, time, instance, msg)) {
					return false;
				}
			...
			if (time > depot.dueDate) {
				if (msg) std::cerr << "Arrived at Depot after Due Date." << std::endl;
				return false;
			}
		}
	}
...
bool validator::ConstraintsRouting::checkTimeWindowCustomer(const unsigned int customer_id, const unsigned int customer_last_id, double& time, const Instance& instance, const bool& msg) {
	...
	if (time > customer_current.dueDate) {
		if (msg) std::cerr << "Arrived at Customer " << customer_id << "after Due Date." << std::endl;
		return false;
	}
...
bool validator::ConstraintsRouting::checkCapacities(const Solution& solution, const bool load_capacity, const Instance& instance, const bool& msg) {
	for (const auto& tour : solution.tours) {
		...
		if (sumVolume > instance.vehicle.load_volume) {
			if (msg) std::cerr << "Exceeding Volume in Tour " << tour.id << std::endl;
			return false;
		}
		if (sumMass > instance.vehicle.D && load_capacity) {
			if (msg) std::cerr << "Exceeding Mass in Tour:  " << tour.id << std::endl;
			return false;
		}
	}
	return true;
...
bool validator::ConstraintsRouting::checkDispatchedCustomers(const Solution& solution, const bool split, const Instance& instance, const bool& msg) {
	...
	if (dispatched_customers.size() < instance.customers.size() - 1) {
		if (msg) std::cerr << "Not all Customers are dispatched." << std::endl;
		return false;
	}
...
bool validator::ConstraintsRouting::checkUsedVehicles(const Solution& solution, const Instance& instance, const bool& msg) {
	// Not an error since Constraints can lead to an exceedence
	if (solution.tours.size() > instance.v_max) {
		if (msg) std::cerr << "Number of used vehicles increases number of available vehicles." << std::endl;
	}
	return true;
}
